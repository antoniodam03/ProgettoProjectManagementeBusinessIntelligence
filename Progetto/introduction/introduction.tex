...
\textcolor{red}{\textbf{Da modificare prima e dopo pulizia}}
...
\begin{table}[htbp] % 'htbp' cerca di piazzarla qui, in alto, in basso o in una pagina separata
    \centering
    \footnotesize % Riduce leggermente il font per farla entrare in una pagina
    \caption{Elenco completo delle 27 variabili selezionate dal GTD.}
    \label{tab:dataset_description}
    
    % Definisce la larghezza: l = left, p = paragrafo con larghezza fissa
    \begin{tabular}{|l|p{0.70\textwidth}|}
        ...
        \texttt{eventid} & Identificativo univoco numerico dell'evento (Chiave Primaria). \\
        \hline
        ...
        \hline
        \texttt{region\_txt} & Macro-regione geografica (es. Medio Oriente). \\
        \hline
        \texttt{provstate} & Nome della Provincia o dello Stato amministrativo. \\
        \hline
        ...
        % --- CRITERI ---
        \texttt{Crit1} & Indicatore binario (1/0) violenza o minaccia di violenza. \\
        \hline
        \texttt{Crit2} & Indicatore binario (1/0) perpetrato da gruppo non statale. \\
        \hline
        \texttt{Crit3} & Indicatore binario (1/0) scopo politico, religioso o sociale. \\
        \hline
        
        % --- 3. TATTICHE E ARMI ---
        \texttt{attacktype1\_txt} & Categoria principale della tattica d'attacco. \\
        \hline
        \texttt{weaptype1\_txt} & Categoria generale dell'arma utilizzata. \\
        \hline
        \texttt{weapsubtype1\_txt} & Sottocategoria specifica dell'arma. \\
        \hline
        \texttt{suicide} & Indicatore binario (1/0) se l'attacco è stato suicida. \\
        \hline
        \texttt{success} & Indicatore binario (1/0) successo dell'attacco. \\
        \hline
        
        % --- 4. OBIETTIVI ---
        \texttt{targtype1\_txt} & Categoria generale del bersaglio (es. Civili). \\
        \hline
        \texttt{targsubtype1\_txt} & Sottocategoria specifica del bersaglio. \\
        \hline
        \texttt{corp1} & Nome dell'ente, azienda o gruppo colpito. \\
        \hline
        \texttt{natlty1\_txt} & Nazionalità delle vittime colpite. \\
        \hline
        
        % --- 5. GRUPPI E TERRORISTI ---
        \texttt{gname} & Nome del gruppo terroristico responsabile. \\
        \hline
        \texttt{claimed} & Indicatore binario (1/0) rivendicazione ufficiale. \\
        \hline
        \texttt{nperps} & Numero di terroristi partecipanti. \\
        \hline
        ...
        \hline
        \texttt{propvalue} & Valore stimato del danno economico (USD). \\
        \hline
        \texttt{ishostkid} & Indicatore (1/0) rapimento/presa ostaggi. \\
        \hline
...
\begin{enumerate}
    ...
    Alcuni record presentavano giorno o mese sconosciuti, indicati con valore ``0''. Per garantire la compatibilità con Qlik Sense, Tableau e Power BI, è stata creata una \textit{colonna unificata \texttt{date}}, combinando anno, mese e giorno. I valori sconosciuti sono stati sostituiti con ``1'', evitando errori nella lettura e permettendo aggregazioni temporali coerenti.

    \item \textit{Normalizzazione dei valori numerici:} \\
    Alcune colonne numeriche, come numero di morti, feriti, terroristi coinvolti, danni economici e riscatti richiesti, contenevano il codice ``-99'' a indicare valori sconosciuti. Tutti questi valori sono stati sostituiti con ``0''. Inoltre, eventuali celle vuote (\textit{NaN}) nelle stesse colonne sono state anch’esse convertite in ``0''. Questo passaggio garantisce la consistenza dei dati e permette di calcolare correttamente KPI come morti totali, danni economici complessivi e impatto degli attacchi.
...
Per preparare il dataset del \textit{Global Terrorism Database} per l'importazione nei software di Business Intelligence, è stato utilizzato Python con la libreria \texttt{pandas}. Di seguito sono descritti i principali passaggi eseguiti.
...
columns_of_interest = [
    "eventid", "country_txt", "region_txt", "provstate", "city",
    "latitude", "longitude", "crit1", "crit2", "crit3",
    "attacktype1_txt", "weaptype1_txt", "weapsubtype1_txt",
    "suicide", "success", "targtype1_txt", "targsubtype1_txt",
    "corp1", "natlty1_txt", "gname", "claimed", "nperps",
...
\end{verbatim}
    ...
    \item \textit{Pulizia dei valori numerici:} sostituzione di \texttt{-99} e valori mancanti con \texttt{0}.
\begin{verbatim}
numeric_cols = ["nkill", "nwound", "propvalue", "ransomamt", "nperps"]
df[numeric_cols] = df[numeric_cols].replace(-99, 0).fillna(0)
\end{verbatim}
...
\begin{verbatim}
output_path = "/content/gtd_cleaned.csv"
