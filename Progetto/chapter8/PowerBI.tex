...
Oltre alla pulizia, per consentire l'utilizzo dei dati nel modo migliore all'interno delle dashboard diagnostiche, è stato necessario arricchire il modello creando nuove \textit{Colonne Calcolate} e \textit{Misure DAX} (\textit{Data Analysis Expressions}). 
...
Sono stati sviluppati due parametri per quantificare la pericolosità dei gruppi e la gravità degli eventi oltre il semplice conteggio.
...
Sono stati creati tre parametri utili a segmentare gli attori in base alla loro organizzazione e alle scelte operative.
...
\begin{figure}[h]
    ...
    \caption{Cluster Analysis dei Gruppi Terroristici.}
    \label{fig:dash1}
\end{figure}
...
\begin{figure}[h!]
    ...
    \caption{Analisi dei Fattori di Rischio e Key Influencers.}
    \label{fig:dash2}
\end{figure}
...
La visualizzazione, mostrata in Figura \ref{fig:dash2}, è dominata dal grafico \textit{AI}, che classifica i fattori di rischio in ordine di importanza statistica. Ogni fattore è accompagnato da un moltiplicatore che indica l'incremento relativo del rischio. Sulla destra, un grafico a barre in pila al 100\% permette di confrontare la \textit{Proporzione di Attacchi} tra le diverse armi, evidenziando quelle con la maggiore incidenza di stragi. Il grafico dei \textit{Volumi Operativi} fornisce il contesto statistico mostrando la frequenza delle tattiche, considerando solo gli eventi ad alto impatto. Infine, la dashboard contiene diversi filtri, fondamentali per condurre analisi mirate. Nello specifico, troviamo un \textit{filtro Regione}, per poter analizzare i dati in una determinata area geografica, uno slider per il \textit{Periodo di Analisi}, che consente di isolare archi temporali specifici, e i filtri \textit{Tattica di Attacco} e \textit{Gruppo Terroristico}, utili per calare l'algoritmo predittivo sul comportamento di specifiche organizzazioni
...
\begin{figure}[h!]
    ...
    \caption{Analisi del Fallimento e Resilienza dei Target.}
    \label{fig:dash3}
\end{figure}

La visualizzazione è progettata per un confronto globale rapido. Al centro, in Figura \ref{fig:dash3}, domina la \textit{Griglia della Resilienza Globale}, basata sulla tecnica dei \textit{Small Multiples}: una serie di grafici a linee che monitorano l'evoluzione temporale della metrica \textit{\%\_Fallimento} in ogni macro-regione simultaneamente.
Sulla destra, la \textit{Correlazione Strumento/Esito} utilizza un grafico a barre in pila al 100\% per confrontare l'efficacia delle diverse armi, distinguendo tra attacchi \textit{Riusciti} (in grigio) e \textit{Falliti} (in rosso). A sinistra, una matrice analizza il \textit{Costo Umano} degli eventi tecnicamente falliti, evidenziando le vittime registrate anche quando l'obiettivo principale non è stato raggiunto. Infine, la dashboard è equipaggiata con un pannello laterale di filtri interattivi (slicer) che abilitano un'esplorazione dinamica dei dati. Nello specifico, sono presenti un \textit{filtro temporale} sull'anno, per isolare periodi storici specifici, e un filtro per \textit{Gruppo Terroristico} (\textit{gname}), fondamentale per valutare il tasso di fallimento e l'affidabilità tattica di specifiche organizzazioni eversive rispetto al contesto globale.
...
\begin{figure}[h!]
    ...
    \caption{Analisi dell’Impatto Economico e Finanziamento.}
    \label{fig:dash4}
\end{figure}

La dashboard, mostrata in Figura \ref{fig:dash4}, è organizzata per offrire una visione macroeconomica integrata da dettagli sui flussi finanziari:
...
\begin{figure}[h!]
    ...
    \caption{Analisi della Logistica Operativa e Strategia di Comunicazione.}
    \label{fig:dash5}
\end{figure}

Come mostrato in Figura \ref{fig:dash5}, gli elementi presenti sono organizzati per offrire una visione integrata dell'efficacia operativa e della strategia comunicativa:
...
In basso a sinistra, il grafico sulla \textit{Strategia di Comunicazione} analizza la propensione alla rivendicazione per tipologia di bersaglio, distinguendo tra eventi \textit{Rivendicati} e \textit{Non Rivendicati}. 

Infine, una serie di \textit{KPI Operativi} e un indicatore a tachimetro monitorano il volume operativo totale e la percentuale globale di attacchi che hanno ricevuto una rivendicazione ufficiale. In aggiunta, nella parte inferiore destra è presente un \textit{filtro per Regione} che consente di focalizzare dinamicamente l'intera analisi su specifiche aree macro-geografiche (come il Medio Oriente o l'Europa Occidentale), permettendo di osservare come le logiche organizzative e la necessità di fare propaganda si adattino ai diversi contesti territoriali.
