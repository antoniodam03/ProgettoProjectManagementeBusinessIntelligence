...
\textcolor{red}{\textbf{Da ricontrollare }}
\begin{itemize}
    \item \textbf{Dashboard 1:} Analisi Temporale degli Attacchi e delle Vittime (Trend Storici).
    \item \textbf{Dashboard 2:} Previsione del Volume degli Attacchi.
    \item \textbf{Dashboard 3:} Previsione dell’Impatto Economico per Tipo di Attacco.
    \item \textbf{Dashboard 4:} Analisi e Previsione della Letalità per Regione.
    \item \textbf{Dashboard 5:} Previsione dell’Intensità (Vittime) per Tipo di Attacco e Arma.
    \item \textbf{Dashboard 6:} Previsione dei Bersagli (Target) per Regione.
    \item \textbf{Dashboard 7:} Analisi degli Esiti dell’Attacco (Successo vs Fallimento).
\end{itemize}
...
\textcolor{red}{\textbf{Da controllare}}
...
\subsection{Dashboard 4: Analisi e Previsione della Letalità per Regione}
...
Per generare la proiezione futura della letalità, Tableau ha utilizzato un algoritmo di \textbf{Livellamento Esponenziale} (\textit{Exponential Smoothing}). Questo approccio assegna pesi decrescenti nel tempo ai dati storici (1970--2016). Il modello selezionato automaticamente è di tipo \textbf{Aggiuntivo} per il Livello, senza Trend e senza Stagionalità (Modello: \textit{Aggiuntivo, Nessuno, Nessuno}). L'assenza di stagionalità è coerente con la natura del dataset aggregato su base annua.
...
Gli eventi estremi (\textit{outliers}) possono alterare le medie in qualsiasi momento. Il modello suggerisce che i \textit{decision maker} non devono basare la pianificazione solo sulla media attesa ($0,86$), ma devono preparare le risorse di emergenza per l'estremo superiore dell'intervallo di confidenza, ovvero per potenziali picchi di letalità improvvisi.

\textcolor{red}{\textbf{figura dati previsione}}


\subsection{Dashboard 6: Previsione dei Bersagli (Target) per Regione}
...
\textcolor{red}{\textbf{Da cantrollare}}
...
Per la proiezione a 5 anni (2017--2021) dei volumi di attacco sui diversi bersagli, il motore analitico ha utilizzato il Livellamento Esponenziale sui dati storici dal 1970 al 2016. Coerentemente con la granularità annuale del dataset, il software non ha forzato alcuna componente di stagionalità (Modello Stagione: \textit{Nessuna}). Tuttavia, l'aspetto più rilevante non risiede nelle impostazioni generali, ma in come l'algoritmo si è adattato autonomamente alle diverse serie storiche dei singoli target.
...
\begin{enumerate}
    \item \textbf{Istituzioni, Forze dell'Ordine e Business (Hard/Economic Targets):} Per categorie come \texttt{Police}, \texttt{Military}, \texttt{Government} e \texttt{Business}, l'algoritmo ha selezionato un modello di tipo \textit{"Aggiuntivo, Nessuno, Nessuno"} (ovvero con Livello, ma senza Trend). Il coefficiente Beta ($\beta$, che regola la pendenza del trend) è pari a $0,000$. Di conseguenza, il modello prevede una stabilizzazione della minaccia: la stima di crescita per il quinquennio successivo è pari a $0$ rispetto al valore iniziale.
    \item \textbf{Civili e Proprietà Private (Soft Targets):} Per la categoria \texttt{Private Citizens \& Property}, il software ha invece applicato un modello \textit{"Aggiuntivo, Aggiuntivo, Nessuno"}, rilevando un trend di fondo estremamente forte. Il coefficiente $\beta$ è salito a $0,297$, portando il modello a calcolare un incremento stimato impressionante di $+2.084$ attacchi nel quinquennio. Questo dato certifica matematicamente lo spostamento della violenza verso bersagli non difesi.
\end{enumerate}

\paragraph{Analisi delle Metriche di Qualità (Il significato del rating "Scarso")}
Come già osservato nell'analisi della letalità (Dashboard 4), Tableau classifica la qualità complessiva di queste previsioni come \textit{"Scarsa"}. Questa valutazione è il riflesso dell'alta volatilità geopolitica. Esaminando le metriche specifiche:

\begin{itemize}
    \item \textbf{MAPE (Errore Percentuale Medio Assoluto):} Si attesta su valori che vanno dal $27,7\%$ per il settore \textit{Business} fino a un picco del $50,1\%$ per i \textit{Private Citizens}. L'errore così alto sui civili è dovuto all'impennata anomala ed esponenziale degli attacchi nell'ultimo decennio, che rende i dati storici del passato meno affidabili per prevedere l'entità esatta della crescita futura.
    \item \textbf{MASE (Mean Absolute Scaled Error):} I valori oscillano tra $1,01$ e $1,11$. Essendo leggermente superiori a $1$, indicano che, a causa delle brusche oscillazioni (cicli di pace seguiti da improvvise ondate di attentati), il modello di smussamento esponenziale fatica a performare significativamente meglio di una semplice "previsione Naive" (che replicherebbe linearmente l'ultimo dato noto).
\end{itemize}
...
\subsection{Dashboard 7: Analisi degli Esiti dell'Attacco (Successo vs Fallimento)}
...
\begin{itemize}
    ...
    \textit{Interpretazione:} Sebbene le basi militari siano \textit{Hard Targets} (obiettivi fortificati), le truppe operano spesso in zone di conflitto attivo o in scenari asimmetrici, dove sono costantemente esposte ad agguati, ordigni improvvisati (IED) o attacchi suicidi, rendendo la prevenzione assoluta molto complessa.
...
\begin{enumerate}
    ...
    \item \textbf{Vulnerabilità Tattica:} Un aeroporto è un ambiente chiuso e controllato, dove l'attaccante deve superare filtri successivi. Un convoglio militare o un \textit{checkpoint}, al contrario, si trova spesso in campo aperto, dove l'attaccante può sfruttare il fattore sorpresa o la forza bruta.
\end{enumerate}
