...
\section{Fondamenti Statistici dell'Analisi Predittiva (Forecasting)}
\label{sec:fondamenti_statistici}

Per proiettare i trend futuri all'interno delle dashboard, il motore analitico di Tableau utilizza algoritmi di \textbf{Livellamento Esponenziale} (\textit{Exponential Smoothing}). A differenza delle medie mobili semplici, che assegnano lo stesso peso a tutti i dati storici, il livellamento esponenziale attribuisce un peso via via decrescente alle osservazioni più vecchie, rendendo il modello più reattivo ai cambiamenti recenti.

Per garantire una corretta interpretazione dei risultati (\textit{Actionable Insights}) presentati nelle sezioni successive, è fondamentale definire i parametri matematici e le metriche di errore utilizzati per valutare l'affidabilità di ciascuna previsione.

\subsection*{Coefficienti di Livellamento}
I modelli generati si basano su tre coefficienti principali, i cui valori oscillano tra $0$ e $1$. Valori prossimi a $1$ indicano che il modello si adatta rapidamente alle ultime fluttuazioni, mentre valori prossimi a $0$ indicano una maggiore inerzia e aderenza alla media storica:
\begin{itemize}
    \item \textbf{Alpha ($\alpha$) - Livello:} Misura la rapidità con cui il modello aggiorna il valore di base della previsione. Un Alpha basso significa che un improvviso picco anomalo di attacchi non stravolgerà la linea di base futura.
    \item \textbf{Beta ($\beta$) - Trend:} Determina la reattività del modello ai cambiamenti di pendenza (crescita o decrescita). Se Beta è $0$, il modello non rileva alcuna tendenza strutturale in aumento o in diminuzione.
    \item \textbf{Gamma ($\gamma$) - Stagionalità:} Regola il peso dato ai pattern ciclici. Un valore superiore a $0$ certifica che il modello ha matematicamente isolato una ricorrenza stagionale (ad esempio, un aumento sistematico degli attacchi nei mesi estivi).
\end{itemize}

\subsection*{Metriche di Valutazione dell'Errore}
A causa dell'intrinseca volatilità del fenomeno terroristico (caratterizzato da "Cigni Neri" e picchi anomali improvvisi), Tableau spesso classifica la qualità della previsione come "Scarsa". Questa etichetta non invalida l'algoritmo, ma è il risultato della lettura rigorosa delle seguenti metriche:
\begin{itemize}
    \item \textbf{RMSE (Root Mean Square Error)} e \textbf{MAE (Mean Absolute Error):} Esprimono lo scostamento assoluto tra i valori stimati dal modello e i dati reali osservati. Sono misurati nella stessa unità di misura del grafico (es. numero di vittime o dollari di danno).
    \item \textbf{MASE (Mean Absolute Scaled Error):} È un indicatore fondamentale per validare il modello. Confronta l'errore del modello di livellamento esponenziale con quello di un modello \textit{Naive} (un modello elementare che ipotizza che il valore di domani sarà identico a quello di oggi). Un MASE $< 1$ dimostra matematicamente che l'algoritmo applicato è superiore alla predizione di base e ha intercettato un pattern reale.
    \item \textbf{MAPE (Mean Absolute Percentage Error):} Esprime l'errore medio in termini percentuali. È la metrica che penalizza maggiormente i dati sul terrorismo: in serie storiche dove i valori normali sono bassi ma si verificano picchi estremi del $+3000\%$, il MAPE si alza vertiginosamente, spingendo il software a classificare precauzionalmente la qualità del modello come "Scarsa" per avvisare l'analista dell'alta varianza.
\end{itemize}
...
Per confermare matematicamente l'osservazione visiva, sono state applicate \textbf{Linee di Tendenza Polinomiali di 3° grado (Cubiche)}. I parametri statistici mostrati in Figura \ref{Dati_Vittime_Dashboard_1} e in Figura \ref{Dati_Attacchi_Dashboard_1} confermano la robustezza dell'analisi:

\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{chapter8/images/dashboard/Foto_dash_enrico/rosso_dash_1_enrico}
    \caption{Validazione statistica: parametri del modello polinomiale e linea di tendenza per l'andamento delle vittime.}
...
\begin{figure}[h]
    ...
    \caption{Validazione statistica: parametri del modello polinomiale e linea di tendenza per l'andamento degli attacchi.}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item La curva delle Vittime (Figura \ref{Dati_Vittime_Dashboard_1}) ha un coefficiente cubico ($x^3$) di $3,92 \cdot 10^{-8}$.
        \item La curva degli Attacchi(Figura \ref{Dati_Attacchi_Dashboard_1}) ha un coefficiente cubico ($x^3$) di $1,76 \cdot 10^{-8}$.
    \end{itemize}
...
\subsubsection*{Obiettivo dell'Analisi}
Questa dashboard, presente nella Figura \ref{Dashboard_2}, indaga la relazione tra l'intensità dell'attacco (misurata in numero di feriti, \texttt{nwound}) e la sua mortalità (numero di morti, \texttt{nkill}). L'obiettivo è duplice: verificare la correlazione tra le due variabili e prevedere l'evoluzione della letalità media, permettendo confronti geografici mirati.

\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{chapter8/images/dashboard/Foto_dash_enrico/dash_2_enrico}
    \caption{Dashboard 2: Correlazione tra intensità e mortalità degli attacchi con previsione della letalità per regione.}
    \label{Dashboard_2}
\end{figure}
...
Per validare il modello, è stato eseguito un confronto diretto tra \textit{South Asia} e \textit{North America}.

\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{chapter8/images/dashboard/Foto_dash_enrico/dash_2_sa_enrico}
    \caption{Focus South Asia: analisi della concentrazione degli eventi e stima del tasso di letalità.}
...
\begin{figure}[h]
    ...
    \caption{Focus North America: evidenza degli eventi rari ad alta efficienza letale.}
...
\item \textbf{Analisi Qualitativa:}

\begin{itemize}
    \item \textbf{South Asia:} Come mostrato nella Figura \ref{Dashboard_2_South_Asia}, la nuvola di punti nello \textit{Scatter Plot} è estremamente densa, confermando che gli eventi ad alta intensità non sono eccezioni ma ricorrenze statistiche.
    \item \textbf{North America:} Nella Figura \ref{Dashboard_2_North_America}, si vede che il grafico appare quasi "vuoto". La quasi totalità degli eventi si concentra nell'origine; i casi ad alta intensità sono rari (es. 11 settembre).
\end{itemize}


\item \textbf{Analisi Predittiva e Business Insights:}

L'analisi del \textit{Tasso di Letalità Previsto} (rapporto tra morti e feriti stimati) ha rivelato risultati inaspettati visualizzabili nella Tabella \ref{tab:confronto_letalita}:
...
\item\textbf{Conclusioni Strategiche:}
...
\item \textbf{Motore di Forecasting e Parametri di Base}
...
Come mostrato in Figura \ref{Dashboard_2_Dati_Previsione_n1} l'algoritmo ha calcolato i seguenti valori per la previsione:
\begin{itemize}
...
\begin{figure}[h]
    ...
    \caption{Parametri del modello di livellamento esponenziale applicato alla previsione del tasso di letalità.}
    \label{Dashboard_2_Dati_Previsione_n1}
\end{figure}

\item \textbf{Analisi delle Metriche di Qualità}
Nella Figura \ref{Dashboard_2_Dati_Previsione_n2} si nota che il software ha classificato la qualità della previsione come \textit{"Scarsa"}. Tale valutazione non indica un errore di modellazione, ma riflette l'estrema volatilità del fenomeno. Le metriche specifiche chiariscono il quadro:
...
\begin{figure}[h]
    ...
    \caption{Metriche di qualità e indicatori di errore (RMSE, MAE, MAPE) del modello predittivo per la letalità.}
    \label{Dashboard_2_Dati_Previsione_n2}
\end{figure}

\item \textbf{Interpretazione Strategica del Modello}
...
\item \textbf{Motore di Forecasting e Orizzonte Temporale}
...
\item \textbf{Il Cambiamento di Paradigma (Soft Target vs Hard Target)}
L'analisi dei coefficienti del modello (Figura \ref{Dashboard_3_Dati_Previsione_n1}, Figura \ref{Dashboard_3_Dati_Previsione_n2}) rivela una netta spaccatura matematica tra gli obiettivi civili e quelli istituzionali, confermando l'ipotesi visiva di un cambio di strategia del terrorismo moderno:
...
\begin{figure}[h]
    ...
    \caption{Analisi dei coefficienti di tendenza per la stima volumetrica degli attacchi sui diversi bersagli.}
    \label{Dashboard_3_Dati_Previsione_n1}
\end{figure}


\item \textbf{Analisi delle Metriche di Qualità}
Come già osservato nell'analisi della letalità (Dashboard 2), Tableau classifica la qualità complessiva di queste previsioni come \textit{"Scarsa"}. Questa valutazione è il riflesso dell'alta volatilità geopolitica. Esaminando le metriche specifiche (\ref{Dashboard_3_Dati_Previsione_n2}):
...
\begin{figure}[h]
    ...
    \caption{Valutazione delle performance statistiche del modello di previsione applicato ai volumi di attacco.}
    \label{Dashboard_3_Dati_Previsione_n2}
\end{figure}


\item \textbf{Implicazioni Strategiche}
...
\subsubsection*{Obiettivo dell'Analisi}
Mentre le dashboard precedenti hanno esaminato la frequenza, la letalità e la natura dei bersagli, questa visualizzazione mostrata in Figura \ref{Dashboard_4} sposta il focus sull'efficacia delle misure di sicurezza e dell'intelligence antiterrorismo. L'analisi si basa sulla variabile binaria dell'esito dell'attacco: \textbf{Successo} (l'attacco è stato portato a termine come pianificato) o \textbf{Fallimento} (l'ordigno è stato disinnescato, il terrorista intercettato o l'azione è fallita per cause tecniche).

\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{chapter8/images/dashboard/Foto_dash_enrico/dash_4_enrico}
    \caption{Dashboard 4: Evoluzione storica del tasso di fallimento e proporzione degli esiti degli attacchi globali.}
    \label{Dashboard_4}
\end{figure}
...
\subsubsection*{Configurazione della Visualizzazione}
La dashboard mostrata in Figura \ref{Dashboard_5_Previsione_Volume_Attacchi} è costruita attorno a un \textbf{Time-Series Line Chart} continuo:
\begin{itemize}
...
\begin{figure}[h]
    ...
    \caption{Dashboard 5: Previsione del Volume degli Attacchi - Line chart temporale che illustra l'andamento mensile degli attacchi globali dal 1970 al 2017, con proiezione predittiva per il 2018.}
...
I dettagli matematici di questa elaborazione sono sintetizzati nella \textbf{Figura \ref{fig:dashboard_5_previsione_volume}}. La tabella di riepilogo illustra sia i coefficienti di livellamento scelti dall'algoritmo (che confermano matematicamente l'assenza di trend e stagionalità tramite l'azzeramento dei parametri $\beta$ e $\gamma$), sia le metriche di scostamento ed errore (tra cui RMSE, MAE e MAPE). Sono proprio questi indicatori a determinare la classificazione "Scarsa" assegnata da Tableau, evidenziando il margine di incertezza fisiologico di un fenomeno altamente volatile.

\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{chapter8/images/dashboard/dash_diomi/dati dash 5.png}
    \caption{Dati previsione Dashboard 5 - Tabella riassuntiva dei coefficienti di livellamento e delle metriche di errore calcolate dal modello predittivo.}
...
\subsubsection*{Configurazione della Visualizzazione}
Per gestire l'enorme sproporzione economica tra diverse tattiche, la dashboard, visibile nella Figura \ref{Dashboard_6_Previsione_Impatto_Economico}, è stata progettata come uno \textit{Small Multiples Line Chart} (Grafico a Linee a Faccette), suddiviso per righe.
...
\begin{figure}[h]
    ...
    \caption{Dashboard 6: Previsione dell'Impatto Economico per Tipo di Attacco - Small Multiples Line Chart che scompone i danni economici storici e le stime future per ciascuna tattica.}
...
\begin{itemize}
    \item \textbf{Analisi dei Coefficienti e delle Metriche di Qualità}
\end{itemize}
I risultati numerici estratti dal modello, consultabili nella Figura \ref{fig:dashboard_6_dati_previsione}, evidenziano l'entità del rischio finanziario per le due tattiche economicamente più impattanti:
\begin{itemize}
...
\begin{figure}[h]
    ...
    \caption{Dati previsione Dashboard 6 - Dettaglio delle stime economiche previste, intervalli di confidenza e metriche di errore per categoria.}
    \label{fig:dashboard_6_dati_previsione}
\end{figure}

\begin{itemize}
    \item \textbf{Lettura Diagnostica delle Metriche di Qualità}
\end{itemize}
Le valutazioni di Tableau ("Ok" per le infrastrutture, "Scarso" per le esplosioni e i dirottamenti) sono perfettamente giustificate dalla lettura congiunta dei parametri precedentemente introdotti:
\begin{itemize}
    \item \textbf{MAPE estremo:} Il parametro che penalizza maggiormente i bombardamenti portandoli a un rating "Scarso" è un MAPE del $4528,8\%$. Questo valore fuori scala indica che i picchi storici sono così violenti e improvvisi da rendere l'errore percentuale medio altissimo rispetto ai periodi di "calma".
    \item \textbf{Coefficiente Alpha (Livello):} Per quasi tutte le tattiche questo valore è bassissimo ($0,000$ o $0,040$). Questo significa che il modello non dà peso all'ultimo trimestre registrato, capendo che un attentato distruttivo oggi non implica necessariamente un attentato domani. L'unica eccezione è \textit{Bombing/Explosion} ($\alpha = 0,180$), che mostra una leggera "memoria a breve termine", confermando che le campagne di bombardamento avvengono spesso a ondate contigue.
...
\subsubsection*{Configurazione della Visualizzazione}
La dashboard, presente in Figura \ref{fig:dashboard_7_previsione_intensita_per_arma}, è composta da due visualizzazioni sovrapposte e interdipendenti:
\begin{enumerate}
...
\begin{figure}[h]
    ...
    \caption{Dashboard 7: Previsione dell'Intensità (Vittime) per Arma - Visualizzazione combinata con Box Plot logaritmico per lo studio della dispersione (in alto) e grafico temporale predittivo (in basso).}
...
\begin{figure}[h]
    ...
    \caption{Dati previsione Dashboard 7 - Riepilogo dei modelli statistici applicati, con particolare evidenza per il modello moltiplicativo stagionale estratto per le armi da fuoco.}
